...
(function (window) {
	...
	function define_CesiumHeatmap() {
		 ...
		 */
		CesiumHeatmap.create = function (cesium, bb, options, id) {
			var instance = new CHInstance(cesium, bb, options, id);
			return instance;
		};
...
CHInstance.prototype.setWGS84Data = function (min, max, data) {
	if (data && data.length > 0 && min !== null && min !== false && max !== null && max !== false) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			// Coge los puntos que haya en data y los devuelve como coordenadas del rectangulo del heatmap
